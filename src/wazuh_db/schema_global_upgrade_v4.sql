...
CREATE TABLE IF NOT EXISTS _belongs (
    ...
    id_group INTEGER,
    group_priority INTEGER NOT NULL DEFAULT (1000 + abs(random() % 1000)),
    UNIQUE (id_agent, group_priority),
...
UPDATE _belongs SET group_priority = (SELECT row_number() OVER(PARTITION BY id_agent ORDER BY id_agent) FROM belongs);
END;
